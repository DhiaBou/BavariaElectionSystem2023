        ...
        for partei, indexx in zip(parteien, range(len(parteien))):
            try:
                if row["Bewerber " + partei] != "-":
                    ...
                    kandidat: Kandidaten = Kandidaten(
                        Vorname=row["Bewerber " + partei].split(", ")[1],
...
def get_kandidaten(stimmkreis_id, partei_id) -> Kandidaten:
    with get_db() as db:
        return (
            db.query(Kandidaten)
            .filter(
                Kandidaten.StimmkreisId == stimmkreis_id,
                Kandidaten.ParteiID == partei_id,
            )
            .one()
        )
...
def get_kandidaten_id_random_from_the_wahlkreis(stimmkreis_id, partei_id) -> Kandidaten:
    with get_db() as db:
        wahlkreis_id = (
            db.query(Stimmkreis)
            .filter(Stimmkreis.StimmkreisId == stimmkreis_id)
            .one()
            .WahlkreisId
        )
        kandidaten = (
            ...
            .join(Stimmkreis)
            .filter(
                Stimmkreis.WahlkreisId == wahlkreis_id, Kandidaten.ParteiID == partei_id
            )
            .all()
        )
        ...
        if row["Schlüsselnummer"] == stimmkreis_id:
            ...
            for partei, indexx in zip(parteien, range(len(parteien))):
                try:
                    ...
                    if row["Bewerber " + partei] != "-":
                        kandidate_id_from_the_stimmkreis = get_kandidaten(
                            stimmkreis_id, indexx + 1
                        ).KandidatID
                        ...
                        for j in range(int(row[f"Erststimmen {partei} 2023"])):
                            stimmzettel[
                                erstimme_index + j
                            ].Erstestimme = kandidate_id_from_the_stimmkreis
                        erstimme_index += int(row[f"Erststimmen {partei} 2023"])

                    # generate zweitstimmen
                    kandidate_id_from_the_wahlkreis = (
                        get_kandidaten_id_random_from_the_wahlkreis(
                            stimmkreis_id, indexx + 1
                        ).KandidatID
                    )
                    ...
                    for j in range(int(row[f"Zweitstimmen {partei} 2023"])):
                        stimmzettel[
                            zweitstimme_index + j
                        ].Zweitstimme = kandidate_id_from_the_wahlkreis
